...
def _contains_all(target: Counter, sub_count: Counter) -> bool:
    """Return True if :sub_counter: contains all elements of :target:"""
...
def min_window(string: str, target: str) -> str:
    ...
    def _min_window(
        ...
        if _contains_all(target_count, sub_string_count):
            ...
            return _min_window(sub_string_count, left+1, right, sub_string)

        elif right == len(string):
            return sub_string
        
        sub_string_count[string[right]] += 1
        return _min_window(sub_string_count, left, right+1, sub_string)

    target_count = Counter([ c for c in target ])
    return _min_window(Counter(), 0, 0, "")
